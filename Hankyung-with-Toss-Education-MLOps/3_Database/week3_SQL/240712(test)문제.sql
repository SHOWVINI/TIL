...
select tbc.CUSTOMER_CD, tbp.POINT_MEMO, tbp.POINT  
from TB_CUSTOMER as tbc
left join TB_POINT as tbp on tbc.customer_cd = tbp.customer_cd;

		-- 연습문제 3. SELECT 조건  고객포인트 테이블에서 고객코드, 포인트내용, 포인트 필드 제목을 한글로 출력한다. 

select tbc.CUSTOMER_CD as 고객코드, tbp.POINT_MEMO as 포인트내용, tbp.POINT as 포인트 
from TB_CUSTOMER as tbc
left join TB_POINT as tbp on tbc.customer_cd = tbp.customer_cd;
...
select tbc.CUSTOMER_CD, tbp.SEQ_NO, tbp.POINT
from TB_CUSTOMER as tbc
	left join TB_POINT as tbp on tbc.customer_cd = tbp.customer_cd
where tbc.CUSTOMER_CD = '2017053' and tbp.SEQ_NO = 2;
...
select tbc.CUSTOMER_CD, tbp.SEQ_NO, tbp.POINT_MEMO, tbp.POINT, tbp.REG_DTTM
from TB_CUSTOMER as tbc
	left join TB_POINT as tbp on tbc.customer_cd = tbp.customer_cd
where tbp.REG_DTTM  LIKE '%2018%' and point BETWEEN 10000 AND 50000;
...
from TB_CUSTOMER
where Total_Point >= 20000 and BIRTH_DAY LIKE '198%';
